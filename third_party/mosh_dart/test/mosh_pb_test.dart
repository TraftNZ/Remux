...
void main() {
  ...
  group('malformed protobuf', () {
    test('unmarshal truncated varint', () {
      // 0x08 = field 1, wire type 0 (varint); 0x80 = continuation bit set
      // but no terminating byte. Should throw FormatException (truncated).
      final data = Uint8List.fromList([0x08, 0x80]);
      expect(
        () => TransportInstruction.unmarshal(data),
        throwsFormatException,
      );
    });

    test('unmarshal oversized length', () {
      // Field 6 (diff), wire type 2 (length-delimited): tag = (6 << 3) | 2 = 0x32
      // Followed by a varint claiming ~1GB length: encode 1073741824 as varint.
      // 1073741824 = 0x40000000
      // varint encoding: 0x80 0x80 0x80 0x80 0x04
      final data = Uint8List.fromList([0x32, 0x80, 0x80, 0x80, 0x80, 0x04]);
      expect(
        () => TransportInstruction.unmarshal(data),
        throwsFormatException,
      );
    ...
    test('unmarshal empty input', () {
      final empty = Uint8List(0);

      // TransportInstruction.unmarshal on empty returns defaults.
      final ti = TransportInstruction.unmarshal(empty);
      expect(ti.oldNum, equals(0));
      expect(ti.newNum, equals(0));
      expect(ti.diff, isNull);

      // unmarshalHostMessage on empty returns empty list.
      final hostInstrs = unmarshalHostMessage(empty);
      expect(hostInstrs, isEmpty);

      // unmarshalUserMessage on empty returns empty list.
      final userInstrs = unmarshalUserMessage(empty);
      expect(userInstrs, isEmpty);
    });
  });
}
