...
import 'package:mosh_dart/mosh_fragment.dart';
...
void main() {
  ...
  group('MoshTransport', () {
    ...
    test('recv rejects wrong direction', () {
      // Build a real encrypted datagram from a client (TO_SERVER direction),
      // then feed it to another client which expects TO_CLIENT direction.
      final sender = MoshTransport.client(ocb);
      sender.sendNew(Uint8List.fromList('test payload'.codeUnits));
      final datagrams = sender.tick();
      expect(datagrams, isNotEmpty);

      // The sender is a client, so datagrams have direction bit 0 (TO_SERVER).
      // A client receiver expects direction bit 1 (TO_CLIENT). Should reject.
      final receiver = MoshTransport.client(ocb);
      for (final dg in datagrams) {
        expect(receiver.recv(dg), isNull);
      }
    });
  });
}
